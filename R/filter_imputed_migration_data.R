...
#' register_imputed data. First, we need to subset the dataframe, so that only the necessary
#' characteristics are taken, and then subset by selecting only years 2015 and 2019.
#' A new column depicting the balance of the population
#' is added for the necessary plots and the map representation.
#'
...
filter_imputed_migration_data <- function(imputed_data){
  ...
  df <- df %>%
    mutate(Change = Percentage > 0)
  df[df$Change == TRUE, "Change"] <- "positive"
  df[df$Change == FALSE, "Change"] <- "negative"
  df
}
